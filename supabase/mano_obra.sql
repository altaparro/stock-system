-- ============================================================
-- Migración: Mano de obra en ventas
-- Ejecutar en Supabase > SQL Editor
-- ============================================================

-- 1. Agregar columnas de mano de obra al encabezado de la venta
alter table public.ventas
  add column if not exists mano_obra_descripcion text,
  add column if not exists mano_obra_monto numeric(12,2) not null default 0;

-- 2. Reemplazar la función para aceptar mano de obra
...
begin
  ...
  insert into ventas (medio_pago_id, total_base, interes_pct, total, mano_obra_descripcion, mano_obra_monto)
  values (p_medio_pago_id, v_total, v_interes, round(v_total * (1 + v_interes / 100.0), 2), p_mano_obra_descripcion, v_monto_obra)
...
grant execute on function public.registrar_venta(bigint, jsonb, text, numeric) to anon, authenticated;
